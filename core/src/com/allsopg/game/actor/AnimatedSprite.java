...
/**
 * Abstract class to extend contains logic for sprite updating, creation and sizing
...
//moved from second constructor to allow animations to be switched from derived class **
    protected void animationInit(Array<TextureAtlas.AtlasRegion> regions, Animation.PlayMode loopType) {
        regions.sort(new RegionComparator());
        animation = new Animation(Constants.FRAME_DURATION, regions, loopType);
    }

//update to animated between textures within atlasregion multiple overloads will call via super
